...
export async function getRelatedNewsByCategory(slug: string) {
  ...
  try {
    const res = await fetch(
      `${STRAPI_URL}/api/news?filters[categories][slug][$eq]=${slug}&populate=*&sort=id:desc`,
    );
    if (!res.ok) {
      console.error("Error API noticias:", res.status);
      return [];
...
export async function getNewsDetail(slug: string) {
  ...
  try {
    const res = await fetch(
      `${STRAPI_URL}/api/news?filters[slug][$eq]=${slug}&populate[author][populate]=*&populate[categories][populate]=*&populate[image][populate]=*`,
      {
        cache: "no-store",
      },
    );
...
export async function getFeaturedNews() {
  ...
  try {
    const res = await fetch(
      `${STRAPI_URL}/api/news?filters[featured][$eq]=true&sort=id:desc&populate=*`,
    );
    if (!res.ok) {
      console.error("Error API noticias:", res.status);
      return [];
